...
/**
 * POST /sponsor-tx
 * Sponsors a transaction by acting as gas payer
 * Expects the user to sign the transaction first
 */
app.post("/sponsor-tx", async (req, res) => {
    try {
        const { txBytes, senderAddress, userSignature } = req.body;
        ...
        const sponsoredTxData = {
            transactionBlockBytes: builtTx,
        ...
        // If user signature provided, use it; otherwise execute without it
        let signatures = [];

        // Sponsor signs the transaction
        const sponsorSig = await sponsorKeypair.signTransaction(builtTx);
        signatures.push(sponsorSig.signature);
...
⚠️  Note: For hackathon demo, messages are USER-PAID
   To enable sponsorship, configure the gas station properly
  `);
});
